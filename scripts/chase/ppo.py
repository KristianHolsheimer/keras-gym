...
cnn = CNN(env, lr=0.001)
actor_critic = km.ConjointActorCritic(cnn, update_strategy='ppo')
buffer = km.caching.ExperienceReplayBuffer.from_value_function(
    actor_critic.value_function, capacity=256, batch_size=32)
...
for ep in range(10000000):
    ...
    if env.ep % 1000 == 0:
        os.makedirs('./gifs/ppo/', exist_ok=True)
        km.utils.generate_gif(
            env=env,
            policy=km.RandomPolicy(env),
            filepath='./gifs/ppo/ep{:08d}.gif'.format(env.ep),
            resize_to=(600, 600),
            duration=500)
