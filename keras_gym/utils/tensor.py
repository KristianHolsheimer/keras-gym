...
def cross_entropy(P, Z, dist_id, allow_surrogate=True):
    ...
    if dist_id == 'beta':
        ...
        p, n = tf.unstack(P, axis=2)  # shapes: [batch, actions_ndim]
        a = n * p
        b = n * (1 - p)
        ...
        cross_entropy = K.switch(
            ...
            lbeta(ab_th) - (a_th - 1) * K.log(p) - (b_th - 1) * K.log(1 - p),

            # otherwise, use the exact cross-entropy
            lbeta(ab_th) - (a_th - 1) * digamma(a) - (b_th - 1) * digamma(b) \
...
def entropy(Z, dist_id, allow_surrogate=True):
    ...
    if dist_id == 'beta':
        ...
        entropy = K.switch(
            ...
            # if so, use ordinary logpmf (digamma ~ log for large args)
            lbeta(ab) - (a - 1) * K.log(p) - (b - 1) * K.log(1 - p),
