...
class MCTSNode(NumActionsMixin, RandomStateMixin):
    ...
    c_puct : float, optional
        ...
        .. math::
            ...
            Q(s,a)\\ &=\\ \\frac1{N(s)} \\left(
                v(s) + \\sum_{s'\\in\\text{desc}(s,a)} v(s') \\right)\\\\
    ...
    N : 1d array, dtype: int, shape: [num_actions]
        ...
        The state-action visit count :math:`N(s,a)`. It is related to the state
        visit count :math:`N(s)` through aggregation:
    ...
    D : 1d array, dtype: bool, shape: [num_actions]
        ...
        This contains the 'done' flags for each child state, i.e. whether each
        child state is a terminal state.
    ...
    def play(self, tau=None):
        ...
        s, a, r, done : tuple
            ...
            The return values respectively represent the state observation
            :math:`s`, the last action :math:`a` taken from that state, the
            reward :math:`r` received in the transition, and a boolean flag
            that indicates that either the game has finished or the
    ...
    def backup(self, v, a=None):
        ...
        a : action, optional
            ...
            The action :math:`a` to which to attribute the updated
            value/counts. When backing up from the leaf node, ``a`` must be
            left unspecified.
