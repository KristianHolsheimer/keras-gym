...
from ..value_functions import GenericV, ConnectFourV
...
class ConnectFourPolicy(GenericSoftmaxPolicy, ConnectFourFunctionMixin):
    """
    A specific :term:`updateable policy` for the ConnectFour environment.
    ...
    **adam_kwargs : keyword arguments
    ...
    def _head(self, X, variable_scope):
        ...
        Z_raw = keras.layers.Dense(
            units=self.num_actions,
            kernel_initializer='zeros',
            name=(variable_scope + '/policy/raw_logits'))(X)

        # set logits to large negative values for unavailable actions
        Z_masked = keras.layers.Lambda(
            lambda Z: K.switch(
                self._available_actions, Z, -1e3 * K.ones_like(Z)),
            name=(variable_scope + '/policy/masked_logits'))(Z_raw)
    ...
    def _init_models(self):
        ...
        # extract available-actions mask
        self._available_actions = keras.layers.Lambda(
            lambda S: K.cast(S[:, 0, :, 0], 'bool'), 'policy/action_mask')(S)
...
class ConnectFourActorCritic(ActorCritic, ConnectFourFunctionMixin):
    """
    A specific :term:`actor-critic` for the ConnectFour environment.
    ...
    **adam_kwargs : keyword arguments
    ...
    def _policy_head(self, X, variable_scope):
        return ConnectFourPolicy._head(self, X, variable_scope)

    def _value_head(self, X, variable_scope):
        return ConnectFourV._head(self, X, variable_scope)
